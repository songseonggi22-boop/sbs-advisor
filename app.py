...
# 시간 표시 정규화 (9시→09:00~, 11시→11:00~ 등)
_TIME_DISPLAY = {
    "9시":          "09:00~12:00",
    "11시":         "11:00~14:00",
    "14시":         "14:00~17:00",
...
def load_timetable_data() -> dict:
    """시간표.xlsx 파싱 → {'평일': [...], '주말': [...]}
    각 항목: {dept, time, year, month, course}
    """
    ...
    for i, (y, m) in enumerate(zip(row1w, row2w)):
        if y and "년" in str(y):
            cur_year = int(str(y).replace("년", ""))
        if m and "월" in str(m) and i >= 3:      # D열(인덱스3)부터 월 데이터
...
def get_schedule_map(data: list[dict], dept: str, time_slot: str) -> dict:
    """(year, month) → course_name 딕셔너리 반환."""
...
def build_schedule_excel(
    student_name: str,
    entries: list[dict],   # [{type, dept, time, schedule: {(y,m): course}}]
) -> bytes:
    """개인 시간표 Excel 생성 (템플릿 스타일 준수)."""
    ...
    def cell_font(size=10):
        return Font(name="맑은 고딕", size=size)
    ...
    ws.cell(3, 1).value     = "구분"
    ws.cell(3, 1).font      = header_font(11)
    ws.cell(3, 1).alignment = center_align()
    ws.cell(3, 1).fill      = label_fill
    ws.cell(3, 1).border    = border_all
    ws.cell(3, 1).font      = Font(name="맑은 고딕", size=11, bold=True, color="FFFFFF")
    ...
    for entry in entries:
        label_text = (
            f"[{entry['type']}]\n"
            f"{entry['dept']}\n"
            f"{_TIME_DISPLAY.get(entry['time'], entry['time'])}"
        )
    ...
    # ── 비고 행 ───────────────────────────────────────────────────────────────
    ws.row_dimensions[cur_row + 1].height = 20
    cur_row += 2
    ...
    note_cell             = ws.cell(cur_row, 2)
    note_cell.value       = ""
    note_cell.font        = cell_font(11)
...
with st.sidebar:
    ...
    menu = st.radio(
        "메뉴",
        ["📄 패키지 견적서", "✍️ 블로그 자동화", "📅 예약 대시보드", "🗓️ 시간표 생성"],
...
if menu == "🗓️ 시간표 생성":
    st.markdown("""
<div style="background:linear-gradient(120deg,#1e3a5f 0%,#1F4E79 60%,#2563a8 100%);
            padding:1.2rem 2rem;border-radius:14px;margin-bottom:1.4rem;
            box-shadow:0 4px 24px rgba(31,78,121,.35);">
  <h2 style="margin:0;color:#fff;font-size:1.4rem;font-weight:900;">🗓️ 개인 시간표 생성</h2>
  <p style="margin:.25rem 0 0;color:rgba(255,255,255,.80);font-size:.9rem;">
    수강생 이름과 과정을 선택하면 개인 맞춤 시간표 엑셀 파일을 자동 생성합니다
  </p>
</div>
""", unsafe_allow_html=True)
    ...
    if not tt_data["평일"] and not tt_data["주말"]:
        st.error("⚠️ 시간표.xlsx 파일을 찾을 수 없습니다. 프로젝트 폴더에 파일을 확인해 주세요.")
    else:
        # ── 수강생 이름 ──────────────────────────────────────────────────────
        st_name = st.text_input(
            ...
            placeholder="홍길동",
            value=s_name or "",
            help="생성될 엑셀 파일 제목에 사용됩니다.",
        )

        st.markdown("---")
        st.markdown("#### 📋 수강 과정 선택")
        st.caption("평일·주말 각각 최대 3개 과정까지 선택할 수 있습니다.")

        entries: list[dict] = []

        # ── 과정 선택 헬퍼 ───────────────────────────────────────────────────
        def _course_selector(slot_key: str, sheet_type: str, sheet_data: list[dict]):
            """학과 → 시간 드롭다운 UI + schedule 반환."""
            ...
            if schedule:
                ...
                preview_rows = [
                    {"연도": y, "월": f"{m}월", "과정": c}
                    for (y, m), c in schedule.items()
                ]
                with st.expander(f"📅 {dept} / {time_sel} 커리큘럼 미리보기 ({len(schedule)}개월)", expanded=False):
                    st.dataframe(pd.DataFrame(preview_rows), hide_index=True, use_container_width=True)

            return {"type": sheet_type, "dept": dept, "time": time_sel, "schedule": schedule}

        # ── 평일 ─────────────────────────────────────────────────────────────
        with st.expander("📌 평일 과정 선택", expanded=True):
            for idx in range(3):
                st.markdown(f"**평일 과정 {idx+1}**")
                entry = _course_selector(f"wd_{idx}", "평일", tt_data["평일"])
                if entry and entry["schedule"]:
                    entries.append(entry)
                if idx < 2:
                    st.markdown("<hr style='margin:.5rem 0;border-color:#e5e7eb'>", unsafe_allow_html=True)

        # ── 주말 ─────────────────────────────────────────────────────────────
        with st.expander("📌 주말 과정 선택", expanded=False):
            for idx in range(3):
                st.markdown(f"**주말 과정 {idx+1}**")
                entry = _course_selector(f"we_{idx}", "주말", tt_data["주말"])
                if entry and entry["schedule"]:
                    entries.append(entry)
                if idx < 2:
                    st.markdown("<hr style='margin:.5rem 0;border-color:#e5e7eb'>", unsafe_allow_html=True)
        ...
        if st.button("📥 시간표 엑셀 생성", type="primary", use_container_width=True):
            if not st_name.strip():
                ...
                with st.spinner("엑셀 파일 생성 중..."):
                    xlsx_bytes = build_schedule_excel(st_name.strip(), entries)

                safe_name = re.sub(r'[\\/:*?"<>|]', "_", st_name.strip())
                fname = f"SBS_{safe_name}_시간표_{datetime.now().strftime('%Y%m%d')}.xlsx"

                st.success(f"✅ **{st_name}** 수강생 개인 시간표가 생성되었습니다!")

                # 선택 과정 요약
                summary_html = ""
                for e in entries:
                    disp_time = _TIME_DISPLAY.get(e["time"], e["time"])
                    months_list = [f"{y}년 {m}월" for (y, m) in sorted(e["schedule"].keys())]
                    start = months_list[0] if months_list else "-"
                    end   = months_list[-1] if months_list else "-"
                    summary_html += (
                        f"<li><b>[{e['type']}]</b> {e['dept']} · {disp_time} "
                        f"— {start} ~ {end} ({len(e['schedule'])}개월)</li>"
                    )

                st.markdown(
                    f"<div style='background:#eff6ff;border:1.5px solid #3b82f6;"
                    f"border-radius:10px;padding:1rem 1.2rem;margin:.8rem 0'>"
                    f"<b>📋 선택 과정 요약</b><ul style='margin:.5rem 0 0;padding-left:1.4rem'>"
                    f"{summary_html}</ul></div>",
                    unsafe_allow_html=True,
                )
