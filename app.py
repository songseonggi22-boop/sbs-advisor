...
CATEGORY_MAP: dict[str, list[str]] = {
    "그래픽디자인": [
        "공통","일러스트","포토웍스","디일러","아트웍",
        "디지털드로잉포트폴리오","발상과표현","색채학","해부학",
        "스토리보드","광고편집","인포그래픽","편집포트폴리오","그래픽아트웍",
    ],
    "실내건축": [
        "인테리어","캐드/주말","맥스","스케치/제도","시공실무",
        "인테리어포트폴리오","스케치업","BIM","퓨전360",
    ],
    "웹디자인": ["웹","UIUX","웹포폴"],
    "3D·애니메이션": ["CG","마야포트폴리오","블렌더"],
    "영상·모션그래픽": [
        "영상","에펙","모션에펙","어드벤스에펙",
        "시포디","모션포트폴리오","유튜브크리에이터",
    ],
    "프로그래밍": ["프로그래밍","C언어","파이썬"],
    "사무·자격증": [
        "사무OA자격증","컴활2급","컴활1급","전산세무2급, 전산회계1급",
        "재경관리사","컴퓨터기초","자격증","GTQi",
        "웹디자인개발기능사","전산응용건축제도기능사",
        "전산응용기계제도기능사","컴퓨터그래픽기능사","컬러리스트기사/산업기사",
    ],
    "AI 크리에이터": [
        "AI프롬프트엔지니어링","AI디자인",
        "AI크리에이터 - 아트웍","AI크리에이터 - 영상모션",
        "AI크리에이터 - 웹","AI크리에이터 - 유튜브",
        "AI크리에이터 - 인테리어","AI크리에이터 - CG/마야",
    ],
}

COURSE_TO_CAT: dict[str, str] = {
    c: cat for cat, cs in CATEGORY_MAP.items() for c in cs
}

PRESETS: dict[str, dict] = {
    "⚡ 영상·모션 최소": {
        "courses": ["영상","에펙","모션에펙"],
        "desc": "빠른 취업 목표 | 3개 과정 | 약 3~4개월",
    },
    "🚀 영상·모션 최대": {
        "courses": ["공통","일러스트","아트웍","영상","에펙","모션에펙","어드벤스에펙","시포디","모션포트폴리오"],
        "desc": "풀스펙 포트폴리오 | 9개 과정 | 약 10~12개월",
    },
    "✏️ 그래픽 최소": {
        "courses": ["공통","포토웍스","일러스트"],
        "desc": "디자인 기초 입문 | 3개 과정 | 약 3개월",
    },
    "🏠 인테리어 최소": {
        "courses": ["인테리어","캐드/주말","스케치업"],
        "desc": "인테리어 입문 | 3개 과정 | 약 3~4개월",
    },
    "🌐 웹디자인 풀": {
        "courses": ["공통","포토웍스","웹","UIUX","웹포폴"],
        "desc": "웹 퍼블리셔 완성 | 5개 과정 | 약 5~6개월",
    },
    "🤖 AI 크리에이터": {
        "courses": ["AI프롬프트엔지니어링","AI크리에이터 - 영상모션","AI크리에이터 - 아트웍"],
...
def load_courses() -> list[dict]:
    ...
    if not json_path.exists():
        try:
            ...
            st.warning(f"크롤링 실패 ({e}). 샘플 데이터로 실행합니다.")
            return _sample()
    data = json.loads(json_path.read_text(encoding="utf-8"))
    return data if data else _sample()


def _sample() -> list[dict]:
    return [
        {"department":"미분류","course":"공통","price":350_000},
        {"department":"미분류","course":"일러스트","price":350_000},
        {"department":"미분류","course":"포토웍스","price":400_000},
        {"department":"미분류","course":"아트웍","price":500_000},
        {"department":"미분류","course":"영상","price":400_000},
        {"department":"미분류","course":"에펙","price":450_000},
        {"department":"미분류","course":"모션에펙","price":450_000},
        {"department":"미분류","course":"어드벤스에펙","price":450_000},
        {"department":"미분류","course":"시포디","price":500_000},
        {"department":"미분류","course":"모션포트폴리오","price":600_000},
        {"department":"미분류","course":"UIUX","price":400_000},
        {"department":"미분류","course":"웹","price":400_000},
        {"department":"미분류","course":"웹포폴","price":500_000},
        {"department":"미분류","course":"AI프롬프트엔지니어링","price":600_000},
        {"department":"미분류","course":"AI크리에이터 - 영상모션","price":500_000},
    ]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**💸 할인 설정**")
    s_discount = st.slider("전체 할인율 (%)", 0, 50, 0, step=5,
                           format="%d%%",
                           help="선택한 모든 과정에 동일하게 적용됩니다.")
...
with col_right:
    ...
    total    = int(df["_final"].sum())  if not df.empty else 0
    ...
    st.metric("선택 과정 수", f"{len(st.session_state.selected_courses)}개")
